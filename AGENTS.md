...
- Survey endpoints include `GET /v1/surveys?session_id=…`, `GET /v1/sessions/:sessionId/insights`, and `POST /v1/sessions/brand-fit-report` (JSON `session_ids`, optional `question_text` and `question_match_mode`: `exact`, `contains`, or `similar`).
